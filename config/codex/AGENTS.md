...
- 長い手順、観点別チェック、繰り返し使う作業は、`AGENTS.md` に詰め込まず、Codex の skills、subagents、project docs、`.agentops` へ分ける。
- `agentops` の `rules/`、`skills/`、`workflows/` は候補カタログであり、そのまま全量コピーしない。`templates/codex/` と公式 docs を使い、実環境に合わせて採用、調整、見送りを判断する。
