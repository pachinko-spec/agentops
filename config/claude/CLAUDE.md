...
7. ドキュメントを更新する。
8. 差分、検証結果、ドキュメント更新を自己レビューする。
9. 必要な修正を行った場合は、修正後に必ず再レビューする。
10. commit / push / GitHub PR 作成へ進む。
11. PR レビュー後、修正した場合は最終レビューを再実行する。
12. GitHub上でマージする。ローカル main への merge だけで完了扱いしない。
13. マージ後は main に戻り、remote の main を取得して同期確認まで行う。
