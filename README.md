...
このプロンプトを受け取ったら、まずこの `agentops` リポジトリの README、config、docs、rules、skills、workflows を読み込み、設計思想と運用方針を理解する。そのうえで、対象CLIの実グローバル設定、現在の環境、MCP設定、plugin/skill/subagent/hook設定、shell profile、GitHubリモート設定との差分を調査し、グローバル設定反映計画をユーザーに提示する。実設定への書き込み、MCP/plugin導入、shell profile変更、外部反映は、計画提示とユーザー承認の後に行う。
