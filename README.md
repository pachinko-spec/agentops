...
このリポジトリで作業する Codex 向けのプロジェクト固有指示は `AGENTS.md` に置きます。特に `/home/otaku/agentops/.agentops/` と `~/.codex/.agentops/` の記録先の使い分けは、ルートの `AGENTS.md` を優先してください。
...
`-- .agentops/   このリポジトリ自身の作業計画、履歴、引き継ぎ
```
