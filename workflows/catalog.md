...
| project-localize | 既存プロジェクトの設計痕跡 inventory + 4 戦略 (greenfield / inventory-rebuild / coexistence / freeze) を意思決定し、project-intake の前段に置く | 新規 onboarding、グローバル設計改訂後の見直し | 対象 project の `.agentops/plans/current.md` + 該当 CLI のグローバル `.agentops/runs/<run-id>/inventory.md` (Claude Code: `~/.claude/.agentops/runs/`、Codex: `~/.codex/.agentops/runs/`) | [project-integration-policy](../rules/catalog.md) |
...
| notification-cron-setup | Discord webhook 4 channel (DAILLY/WEEKLY/MONTHLY/ANT_TIME) の cron / systemd timer 設定、SessionStart/End / PermissionRequest 待ちの hook 連携、既存 cron 整理手順、SECRET 管理を整理する | 通知運用導入、既存 cron 棚卸し | crontab / systemd timer / hook | [notification-policy](../rules/catalog.md) |
