...
| cross-review | 主エージェントとは別 CLI / 別モデルファミリーの frontier reviewer に確認を依頼し、採否を主 orchestrator が判断する | 高リスク変更、新機能、リファクタ、依存追加、API 契約変更、デプロイ影響、レビュー修正後 | `.agentops/reviews/` / `.agentops/runs/` |
