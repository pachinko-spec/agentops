...
- **skill / workflow 側からの逆参照**: skills/catalog.md と workflows/catalog.md に `関連 rule（代表）` 列を追加 → **plan `2026-04-29-handoff-followups` task 01 で実装済み**。代表 1 件方式（複数該当時は最も近い rule を採用）。
...
- **last-reviewed frontmatter**: task 04（P1-04）で他 docs と同形式の YAML frontmatter を一括追加（前 plan で完了済）。catalog 側の frontmatter 追加は handoff 候補。
