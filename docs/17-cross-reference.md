...
| notification-policy | Discord webhook の channel 区分・SECRET 管理・ANT_TIME 頻度上限・SessionStart/End / PermissionRequest 待ちの発火境界を統一する | [notification-digest-writer](../skills/catalog.md) | [notification-cron-setup](../workflows/catalog.md) | — |
| auto-merge-permission | AI auto-merge 許諾条件 6 (全 AND)・post-merge 1 PR scope 完結原則・「user 明示許可」3 要件例外条項を定義する | [release-readiness-review](../skills/catalog.md) | [feature-delivery](../workflows/catalog.md) | グローバル `~/.claude/hooks/_common.py` の `inspect_agentops` (本 repo に hook 実体なし、`agentops-task-policy` と同経路) |
| session-record-and-handoff | `.agentops/` 責務・運用フロー・セッション分割条件・prompts/next-session.md 同セッション内更新原則 (hook 整合) を定義する | [session-handoff](../skills/catalog.md) | [session-handoff](../workflows/catalog.md) | グローバル `~/.claude/hooks/_common.py` の `inspect_agentops` (本 repo に hook 実体なし、`agentops-task-policy` と同経路) |
...
- **auto-merge-permission / session-record-and-handoff**: グローバル `~/.claude/hooks/_common.py` の `inspect_agentops` が Stop 時に completed task 残存を block (`"before finalizing"` message) し、post-merge 1 PR scope 完結原則と同セッション内整理原則を強制する。本 repo は hook 実体を持たない方針 ([docs/09-hooks-quality-gates.md](./09-hooks-quality-gates.md)) のため、agentops repo 上では `scripts/agentops archive task` の手動実行と AI auto-merge 許諾条件 (DbC / cross-review / CI / 観察事実 / scope / secret) のレビュー時確認に依存する。
- **notification-policy**: hook event そのもので違反を検知する rule ではなく、SessionStart / Stop / PermissionRequest hook 経由で `agentops-watch notify --kind <kind>` を呼ぶ通知系。仕様メモは [`templates/claude/hooks/session-notify-stub.md`](../templates/claude/hooks/session-notify-stub.md) にあるが、本 repo は実 hook を持たない方針 (詳細は [docs/09-hooks-quality-gates.md](./09-hooks-quality-gates.md)) のため、hook 列は `—` のまま。実 hook 体は対象 CLI のグローバル設定 (`~/.claude/hooks/` / `~/.codex/hooks/` 等) で運用する。
