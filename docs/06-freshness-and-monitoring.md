...
通知の channel 区分・kind 対応・payload 雛形・SECRET 管理・頻度上限ガード・公式 Discord plugin との棲み分けは [通知戦略](18-notification-strategy.md) に集約する。実体 CLI の起動仕様は [監視CLI仕様](11-monitoring-cli.md) を参照する。

通知方針 (上位):

- 毎日: 全プロジェクトの作業状況ダイジェスト (`DAILLY` channel、`--kind daily`)
- 毎週月曜: 週次集計と完了 plan 一覧 (`WEEKLY` channel、`--kind weekly`)
- 月初: 月次集計と archive サマリ + 陳腐化チェック対象 (`MONTHLY` channel、`--kind monthly`)
- 随時: SessionStart/End、PermissionRequest 待ち、Stop failure、任意のアラート (`ANT_TIME` channel)

旧 envvar `AGENTOPS_DISCORD_WEBHOOK_URL` は deprecated。新規 cron / 新規 hook は `DISCORD_WEBHOOK_URL_{DAILLY,WEEKLY,MONTHLY,ANT_TIME}` の 4 本立てへ移行する (詳細は docs/18)。
