...
DbC はすべてのタスクで使う軽量な契約である。harness は、DbC だけでは再現条件が不足する作業に使う拡張契約である。
...
- Claude Code docs: [settings](https://code.claude.com/docs/en/settings)、[hooks](https://code.claude.com/docs/en/hooks)、[subagents](https://code.claude.com/docs/en/sub-agents)、[skills](https://docs.claude.com/en/docs/agents-and-tools/agent-skills) は、project local な permission と lifecycle hook の実装先になる。
