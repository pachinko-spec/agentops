...
加えて、`rules/`、`skills/`、`workflows/` は agentops 保守専用に寄りすぎると、実プロジェクトのコード開発で使いにくくなる。グローバル設定に投影するテンプレートとして、`~/dev` 配下の Web システム開発、Cloudflare / Xserver / GCP / ローカルサーバーへのリリース、運用と収益化まで扱える粒度を保つ必要がある。
