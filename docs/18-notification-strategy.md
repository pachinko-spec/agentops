...
| --- | --- | --- | --- |
| `daily` | DAILLY | cron (例: `10 9 * * *`) | 当日の未完了 task / handoff / next-session.md / dirty worktree / stuck run の集計 |
| `weekly` | WEEKLY | cron (例: `45 9 * * 1`) | 週次集計 + 直近 7 日の完了 plan / 残課題 |
| `monthly` | MONTHLY | cron (例: `0 11 1 * *`) | 月次集計 + archive サマリ + freshness-sources.yml の陳腐化候補 |
