...
- **2026-05-01 更新** (catalog 参照形式): `coding_frontier` / `coding_fast` / `docs_agent` の Codex 候補を反映先 catalog (`~/.claude/agentops/model-catalog.yml`) で再揃えしたケース。具体 model id は catalog を参照し、本 docs 本文には書かない (`~/.claude/rules/global-content-boundary.md` の例外節「機械可読 spec は固定値 OK、本文 markdown には固定 model id を埋めない」と整合)。**frontier 採用と baseline 化の並走時に発生した trim 事故**を是正し、cross-review の別系列原則を catalog 上で機械可読に維持する判断。
...
durable instructions / catalog / AGENTS.md / global rules / migration / security / public API / 課金 / deploy 影響を持つ plan では、5 工程で進める。
