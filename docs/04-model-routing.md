...
- `*_frontier` 系の Codex 候補 (orchestrator / architect / review): cross-review reviewer として **GPT 系の高推論モデル (例: `gpt-5.5` 系)** を採用したケース。注意点として、この系列は ChatGPT auth 限定の場合があり、API key auth 環境では下位モデルへ override されることがある。
