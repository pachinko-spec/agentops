...
agentops は、Claude Code / Codex のグローバル設定に反映する設計思想と参照テンプレートを保守するリポジトリである。

このリポジトリ自体が日々のアプリケーション開発対象ではない。ただし `rules/`、`skills/`、`workflows/` は、グローバル設定へ取り込まれた後、`~/dev` 配下の実プロジェクトで使われることを前提に設計する。
