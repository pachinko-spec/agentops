...
- README と主要 docs の位置づけを、グローバル設定時の参照キットへ寄せた。
- `decisions/`、`.agentops/plans/`、`.agentops/task-plans/`、`.agentops/tasks/` の役割を分けた。
- `docs/15-reference-kit-structure.md` に `rules/`、`skills/`、`workflows/` の分類案、移行方針、archive 方針、参照切れ確認方法を記録した。
- 005 では参照キット本体の大規模移動や削除を行わず、実移動はユーザー承認後に分ける方針にした。
- 完了済み task と task-plan は `.agentops/archive/2026-04-27-agentops-reference-kit-refactor/` へ移す。
