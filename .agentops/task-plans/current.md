...
| 1.2 | docs/17 と整合する rule マッピング決定 (代表 1 件、複数なら `/` 区切り) | ✅ 完了 |
| 1.3 | skills/catalog.md 4 セクション (design 10 / impl 4 / review 9 / docs/ops 8) に「関連 rule（代表）」列追加 | ⏳ 進行中 |
| 1.4 | workflows/catalog.md 単一 table (15 workflow) に「関連 rule（代表）」列追加 | ⏳ |
| 1.5 | docs/17 §残課題 §57 行目「skill / workflow 側からの逆参照」を「task 01 で実装済み」に更新 (任意) | ⏳ |
| 1.6 | `.agentops/reviews/01-cross-reference-bidirectional.md` 新規 (Codex Round 1/2/3 枠) | ⏳ |
| 1.7 | ローカル smoke (compileall / unittest / 列数 grep) | ⏳ |
| 1.8 | commit + push + PR 作成 | ⏳ |
| 1.9 | CI 4 job 全 pass 確認 | ⏳ |
| 1.10 | Codex 3 Round (R1 → 修正 → R2 clean → R3 `no further P0/P1`) | ⏳ |
