...
- ~/dev/ 既存 5 プロジェクト (ai-engine / ai-keiba / ai-utg / pachi-studio / ai-content-engine) のうち 4 件が CLAUDE.md/AGENTS.md/.codex 保持
...
2. `~/dev/` 既存 5 プロジェクトに対する dry-run 表が docs/19 に含まれること
3. agentops-reviewer subagent で独立レビュー
4. Codex cross-review
5. P0/P1 反映後 reviewer 再走
6. `scripts/agentops archive task --task-id 02-project-localization-docs` 実行
7. commit → push → PR → auto-merge → main 同期
